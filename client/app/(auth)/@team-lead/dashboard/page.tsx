...
export default function DashboardPage() {
	return (
		<main className="min-w-0 flex-1 overflow-y-auto p-4 md:p-8 lg:px-12">
			<div className="mx-auto flex w-full max-w-300 flex-col gap-8">
				<div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
					<div className="flex flex-col gap-2">
						<h1 className="font-black text-3xl tracking-tight md:text-4xl">Team Alpha</h1>
						<div className="flex flex-wrap items-center gap-2 text-muted-foreground">
							<span className="text-sm">Capstone 2024</span>
							<span className="text-muted-foreground/50">•</span>
							<span className="font-medium text-primary text-sm">Sprint 4 Active</span>
						</div>
					</div>
				...
				<Card>
					<CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
						...
						<div className="hidden text-right sm:block">
							...
							<p className="text-muted-foreground text-sm">Target: 70% by Friday</p>
						</div>
					</CardHeader>
				...
				<div className="grid grid-cols-1 gap-6 md:grid-cols-3">
					{phasesData.map((phase) => (
						<Card
							className={phase.isCurrent ? "relative ring-1 ring-primary/20" : "relative"}
							key={phase.id}
						>
							<CardHeader>
								<div className="flex items-center gap-3">
									<div className={`rounded-lg ${phase.bgColor} p-2 ${phase.textColor}`}>
					...
					<Card>
						...
						<CardContent className="space-y-6">
							<div className="grid grid-cols-3 items-center text-center">
								...
								<div className="space-y-1">
									<div className="font-black text-3xl text-muted-foreground">8</div>
									<div className="text-muted-foreground text-xs uppercase tracking-wide">
										Remaining
									</div>
								</div>
								<div className="space-y-1">
									...
									<div className="text-destructive text-xs uppercase tracking-wide">Blocked</div>
								</div>
							</div>
						...
						<CardContent className="p-0">
							<div className="flex flex-col">
								{activitiesData.map((activity, index) => {
									const iconConfig = activityIconMap[activity.type as keyof typeof activityIconMap];
									const IconComponent = iconConfig.icon;
									return (
										<div
											className={`flex gap-4 p-4 transition-colors hover:bg-muted/50 ${
												index < activitiesData.length - 1 ? "border-border border-b" : ""
											}`}
											key={activity.id}
										>
											<div
