...
function MeetingsFilters({
  ...
  const selectedScopes = (table.getColumn("scope")?.getFilterValue() ?? []) as string[];

  const handleScopeChange = (checked: boolean, value: string) => {
    const newScopes = checked
      ? [...selectedScopes, value]
      : selectedScopes.filter((s) => s !== value);
    table.getColumn("scope")?.setFilterValue(newScopes.length ? newScopes : undefined);
  };
        ...
        <Popover>
          ...
          <PopoverContent align="start" className="w-auto min-w-36 p-3">
            <div className="space-y-3">
              <div className="font-medium text-muted-foreground text-xs">Scope</div>
              <div className="space-y-3">
