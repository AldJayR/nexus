...
import { Columns3Icon, FilterIcon, ListFilterIcon, CircleXIcon } from "lucide-react";
...
export function GenericTableFilters<T>({
	...
	const primaryFilterValue = primaryFilter
		? (table.getColumn(primaryFilter.columnId)?.getFilterValue() as string[]) ?? []
		: [];
	const primaryFilterCount = primaryFilterValue.length;

	const handleToggleFilter = (checked: boolean, value: string, columnId: string) => {
		const current = (table.getColumn(columnId)?.getFilterValue() as string[]) ?? [];
				...
				{config.search.enabled && (
					<div className="relative">
						<Input
							...
							className={
								"peer min-w-80 ps-9" + (Boolean(searchValue) ? " pe-9" : "")
							}
							id={`${filterId}-search`}
						...
						<PopoverTrigger asChild>
							...
							>
								...
								{filterIndex === 0 && primaryFilterCount > 0 && (
									<span
										className="-me-1 inline-flex h-5 max-h-full items-center rounded border bg-background px-1 font-[inherit] text-[0.625rem] font-medium text-muted-foreground/70"
